...
This document serves as a guide for future AI agents and developers maintaining this repository. It outlines critical architectural decisions, common pitfalls in ComfyUI custom node development, and specifics regarding the "Folder Selector" node.

## 1. Cross-Platform Safety (Critical)
**The Issue:** The `folder_selector` node uses `ctypes.windll` and `ctypes.wintypes`. These libraries **do not exist** on Linux or macOS.
...
**The Solution:**
* For this specific node (`Folder Selector`), we explicitly acknowledge it is for **Local Windows Only**.
