...
async def get_append_options(request):
    try:
        config_path = os.path.join(os.path.dirname(__file__), "append_options.txt")
...
async def get_config(request):
    try:
        ...
        logging.error(f"[GoddessLabs] Error reading config: {e}")
        return web.json_response({"error": str(e)}, status=500)
